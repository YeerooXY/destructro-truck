...
- Rear equipment uses a module-specific activation model. Some modules are passive; some provide a player-controlled boost meter that can be spent in shorter bursts during a run; and some provide a single, substantially stronger one-time activation. Exact meter capacity, recharge rules, and module effects will be defined later.
